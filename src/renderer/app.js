  ...
  function handleOperator(nextOp) {
    ...
    if (nextOp === "=") {
      if (
        ...
        setHistory(
          `${formatNumber(state.firstOperand)} ${symbolForOp(state.operator)} ${formatNumber(inputValue)} =`,
        );
    ...
    if (state.operator && state.waitingForSecondOperand) {
      ...
      setHistory(
        `${formatNumber(state.firstOperand)} ${symbolForOp(state.operator)}`,
      );
      return;
    }
